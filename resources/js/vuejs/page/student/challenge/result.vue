<script>
export default {
    props: ['challenge', 'endCourse'],
    methods: {
        next() {
            this.$emit('next')
        },
        tryagain() {
            this.$emit('tryagain')
        }
    },
}
</script>
<template>
    <div class="bg-white" style="padding-bottom: 9rem; padding-left:20rem;">
        <div class="content p-5">
            <div class="card card-quiz text-left" style="margin-top: 3rem">
                <div class="row ">
                    <div class="col-4 d-flex align-items-center">
                        <div class="row p-5">
                            <h3>Your Challenge Result</h3>
                        </div>
                    </div>
                </div>
            </div>
            <div class="box-quiz min-height-quiz">
                <div class="row">

                    <div class="col p-5">
                        <h3 class="mb-4">
                            {{ challenge?.isPassed ? 'Congratulations! You Have Completed This Quiz' :
                                'Unfortunately you didn`t make it through this challenge..' }}
                        </h3>
                        <div class="row d-flex justify-content-start mb-2">
                            <div class="col-3">
                                <h5>Exam Score</h5>
                            </div>
                            <div class="col-3">
                                <h5>Duration (minute)</h5>
                            </div>
                            <div class="col-3">
                                <h5>Hint Obtained</h5>
                            </div>
                            <div class="col-3">
                                <h5>Rating</h5>
                            </div>
                        </div>
                        <div class="row d-flex justify-content-start mb-3">
                            <div class="col-3">
                                <h5>{{ challenge?.isPassed ? 'Correct Answer' : 'Wrong Answer' }}
                                </h5>
                            </div>
                            <div class="col-3">
                                <h5>{{ challenge.time }}
                                </h5>
                            </div>
                            <div class="col-3">
                                <h5>+{{ challenge.score }}
                                </h5>
                            </div>
                            <div class="col-3">
                                <h5 style="font-weight: 900" :class="challenge.isPassed ? 'text-success' : 'text-danger'">
                                    {{ challenge?.isPassed ? 'Passed' : 'Failed' }}
                                </h5>
                            </div>
                        </div>

                        <div class="col-9">
                            <div class="row">
                                <div class="col-2" v-if="!challenge.isPassed"><a @click="tryagain" class="btn btn-button">Try
                                        Again</a>
                                </div>
                                <div class="col-2 me-5" id="endCourse" v-if="endCourse">
                                    <a class="btn btn-button" style="min-width: 130px" href="/student">End Course</a>
                                </div>

                                <div class="col-2" v-if="!endCourse"><a @click="next" id="nextCourse"
                                        class="btn btn-button">Next
                                        Lesson
                                    </a>

                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</template>